...
const prefix = document.querySelector('#form_collection-prefix').innerText;
...
function Ingredients() {
    ...
    const dropdown = (e) => {
        if (e.target.classList.contains('form__item-list')) {
            ...
            cantidadVal.setAttribute('data-id', e.target.getAttribute('data-id'));
        }
    };
    ...
    const addIngredient = (e) => {
        if(nameIngredient.value && cantidad.value) {
            ...
            elem.innerHTML = `<span> ${data.name} ${data.value}${data.units}</span> <span class="form__field-item-delete"></span>
                             <input id="idIngredient_${cur}" name="${prefix}-${cur}-ingredient" type="hidden" value="${data.id}">
                             <input id="nameIngredient_${cur}" name="${prefix}-${cur}-name" type="hidden" value="${data.name}">
                             <input id="valueIngredient_${cur}" name="${prefix}-${cur}-amount" type="hidden" value="${data.value}">
                             <input id="unitsIngredient_${cur}" name="${prefix}-${cur}-units" type="hidden" value="${data.units}">`;
            cur++;
    ...
    const getValue = (e) => {
        const data = {
            id: cantidadVal.getAttribute('data-id'),
    ...
    return api.getIngredients(elem.target.value).then( e => {
        if(e.length !== 0 ) {
            const items = e.map( elem => {
                return `<a class="form__item-list" data-val="${elem.dimension}" data-id="${elem.id}">${elem.title}</a>`
            }).join(' ')
